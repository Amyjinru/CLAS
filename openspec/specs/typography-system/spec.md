## Purpose
建立 CLAS 前端模块化排版系统：基于 1.25（Major Third）比例尺定义 6 级字号层级、统一行高标准、添加正文字行长约束、实现流体排版。
...
- **THEN** 其 `font-size` SHALL 引用 `var(--text-h2)`，`line-height` SHALL 引用 `var(--leading-h2)`
...
- **THEN** `.page-title` 的字号 SHALL 使用 `clamp(22px, 5vw, var(--text-h2))` 在最小值和桌面值之间平滑过渡
