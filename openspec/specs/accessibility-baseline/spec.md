## Purpose
建立 CLAS 前端的无障碍基线：skip-to-content 链接、图标 ARIA 标注、自定义聚焦环、WCAG AA 颜色对比度、表单标签关联，确保键盘用户和屏幕阅读器可正常使用。
...
#### Scenario: 按钮文字对比度
- **WHEN** 主按钮文字（白色）渲染在琥珀色背景上
- **THEN** 对比度 SHALL ≥ 4.5:1，`--color-primary` SHALL 为 `--clas-amber-700`（#c2410c）
