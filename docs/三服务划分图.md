# CLAS 业务微服务 + compat 划分图

> 状态：当前实现（2026-09-01）。正式业务服务为 `clas-iam`、`clas-merchant`、`clas-catalog`、`clas-order`；`clas-compat` 为过渡层（骑手/Admin/公告/聊天）。
> 网关：前端 Nginx 按路径反代（不算业务微服务）。

## 1. 划分原则

- **一表一主写**：其他服务只走 HTTP 内部接口，禁止跨库 JOIN。
- **商家独立**：入驻、审核、资料、营业状态由 `clas-merchant` 主写；商品/预约/团购发布留在 `clas-catalog`。
- **不硬塞**：骑手配送、运营与沟通保持为独立 compat 服务。

## 2. 架构图

```mermaid
flowchart TB
    FE[Vue 前端] --> NGX[Nginx 路径路由]
    NGX --> IAM[clas-iam<br/>账户与触达]
    NGX --> MCH[clas-merchant<br/>入驻审核营业]
    NGX --> CAT[clas-catalog<br/>商品预约团购发布]
    NGX --> ORD[clas-order<br/>交易与评价]
    NGX --> CMP[clas-compat<br/>骑手·运营·沟通]

    ORD -->|HTTP| MCH
    ORD -->|HTTP| CAT
    ORD -->|HTTP| IAM
    ORD -->|HTTP| CMP
    CAT -->|HTTP| MCH
    CAT -->|HTTP| IAM
    MCH -->|HTTP| IAM
    MCH -->|HTTP| CAT
    CMP -->|HTTP 受控查询| IAM
    CMP -->|HTTP 受控查询| MCH
    CMP -->|HTTP 受控查询| CAT
    CMP -->|HTTP 受控查询| ORD

    IAM --- DB[(MySQL clas)]
    MCH --- DB
    CAT --- DB
    ORD --- DB
    CMP --- DB

    IAM --- R[(Redis)]
```

## 3. 业务微服务职责

| 服务 | 端口 | 负责业务 | 为何独立 |
| --- | --- | --- | --- |
| **clas-iam** | 8081 | 注册登录、资料、地址、收藏、银行卡、站内通知、角色申请、用户侧处罚/申诉 | 身份是全系统入口 |
| **clas-merchant** | 8085 | 商家入驻、审核、资料、营业/配送状态、Logo | #38 商家状态调用链；表 `merchant` / `merchant_audit_log` 唯一写服务 |
| **clas-catalog** | 8082 | 商品、分类、**团购发布**、**服务预约**、商品图片 | 供给目录，商家身份经 merchant 内部 API |
| **clas-order** | 8083 | 购物车、订单、支付、退款、**团购购买/核销**、**优惠券**、**订单评价** | 核心交易写路径与状态机 |

## 4. compat 职责（过渡层）

| 模块 | 说明 |
| --- | --- |
| 骑手 / 配送 UC16 | 独立域，表多，与 order 双向联动 |
| `/api/admin` | 跨服务聚合、导出 |
| `/api/announcement` | 平台公告 |
| 沟通 | 主写会话/消息表 |

## 5. 改造版本

| 版本 | Git | 部署形态 |
| --- | --- | --- |
| 改造前 | 标签 `monolith-start` | 单个 `backend` |
| 四进程 | `main` 早期微服务提交 | iam + catalog（含商家）+ order + compat |
| 商家拆分 | 本变更 | iam + **merchant** + catalog + order + compat + Nginx 路由 |
