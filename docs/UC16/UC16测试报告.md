...
| 本地镜像构建与 Compose 运行 | **通过** | 2026-08-28：经 `scripts/pull-base-images.ps1` 绕过 Docker Hub TLS 证书问题后，`docker compose build --pull=false && up -d` 成功；`compose-smoke.ps1` 前端与 `/api/health` 均 PASS。修复项：`.sh` 强制 LF、`migrate` 改用官方 `mysql:8.4.4` 客户端镜像。 |
| 云端 k3s 部署 | **通过** | 2026-08-28 已在 `8.141.112.182` 完成单节点 k3s、Traefik、MySQL PVC、数据库迁移 Job、前后端工作负载和公网 80 端口切换。最新成功部署为 [33184111748](https://github.com/Amyjinru/CLAS/actions/runs/33184111748)，提交 `c7c4cb9e`；`http://8.141.112.182/api/health` 返回 `code=200`、`data=ok`。 |

容器化工作流以 `main` 为唯一自动部署分支，镜像使用 Git SHA 标签而非 `latest`。成功运行见 [33184111748](https://github.com/Amyjinru/CLAS/actions/runs/33184111748)；故意失败门禁记录见 [33172480467](https://github.com/Amyjinru/CLAS/actions/runs/33172480467)，其 publish/deploy 未执行；服务器 GHCR 拉取超时的真实失败记录见 [33178166511](https://github.com/Amyjinru/CLAS/actions/runs/33178166511)，后续已改为 Runner 导出镜像、服务器导入 k3s 的部署方式。每次运行的测试与 k3s 诊断作为 Actions Artifact 留存。
