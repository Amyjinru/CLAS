...
| 骑手一 | `13800000004` | `rider_one` | `Abc123!` |
| 骑手二 | `13800000005` | `rider_two` | `Abc123!` |

### 骑手配送演示与配置

骑手需先拥有已审核的 `RIDER` 身份、上线并上传定位，然后点击“开始接单”领取附近任务；“结束接单”仅关闭新任务领取，已有配送可继续完成。关键接口包括任务池、领取、取餐、送达、订单内沟通、隐私通话、小费、骑手评价、结算与提现。

部署时至少配置：`MYSQL_PASSWORD`、`JWT_SECRET`、`RIDER_IDENTITY_ENCRYPTION_KEY`（32 字节 AES 密钥）与可选 `AMAP_WEB_SERVICE_KEY`。不要将身份证明文、真实用户电话或密钥提交到仓库。

最终答辩建议流程：商家接单 → 骑手上线并开始接单 → 领取/取餐/配送定位 → 用户查看 ETA 并联系骑手 → 骑手送达 → 用户确认、打赏和评价 → 骑手查看收入并申请提现 → 管理员审核提现。
