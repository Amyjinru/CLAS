...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Result<Void>> handleValidationException(MethodArgumentNotValidException exception) {
        String message = exception.getBindingResult().getFieldErrors().stream()
            ...
            .orElse("请求参数错误");
        Result<Void> result = new Result<>(400, message, null, DomainErrorCode.VALIDATION_ERROR);
    ...
    public ResponseEntity<Result<Void>> handleException(Exception exception) {
        ...
        Result<Void> result = new Result<>(500, "系统异常，请稍后重试", null, DomainErrorCode.SYSTEM_ERROR);
