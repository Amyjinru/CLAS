...
admin=admin_r["data"]["token"] if admin_r else None
...
if r and r.get("code")==200:
 ...
 print(f"Reviews total: {len(revs)}")
 for rv in random.sample(revs,min(30,len(revs))):
  ...
  for i in range(20,80):
   ...
   if not t:continue
   rr=api("POST",f"/review/{rid}/report",{"reportReason":"疑似违规内容"},t=t)
   if rr and rr.get("code")==200:rc+=1;break
print(f"Reports: {rc} (skipping own reviews)")
