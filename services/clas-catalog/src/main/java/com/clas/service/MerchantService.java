...
public class MerchantService {
    ...
    public MerchantResponse detail(Long id) {
        ...
        Long favoriteCount = iamClient.favoriteCount(id);
    ...
    public MerchantResponse register(MerchantRegisterRequest request, String loggedInUserId) {
        String finalUserId = iamClient.ensureMerchantApplicant(new MerchantApplicantRequest(
        ...
        if (iamClient.rolesOf(finalUserId).stream().anyMatch(role -> "MERCHANT".equals(role) || "RIDER".equals(role))) {
            throw new BusinessException("已拥有商家或骑手身份，不能申请其他业务身份");
        }
        if (compatClient.hasPendingRiderApplication(finalUserId) || iamClient.hasPendingRiderRoleApplication(finalUserId)) {
    ...
    public MerchantResponse audit(Long merchantId, MerchantAuditRequest auditRequest, String adminId) {
        ...
        if (newStatus == MerchantStatusEnum.APPROVED) {
            InternalUserSummary applicant = iamClient.getUser(merchant.getUserId());
            if (applicant == null) {
                throw new BusinessException("商家申请账号不存在");
            }
            iamClient.grantRole(applicant.userId(), "MERCHANT");
        }
    ...
    private Coordinate resolveCoordinate(BigDecimal latitude, BigDecimal longitude, Long addressId) {
        if (addressId != null) {
            InternalAddressResponse address = iamClient.getAddress(addressId, UserContext.getUserId());
            if (address == null) {
            ...
            return new Coordinate(address.latitude(), address.longitude());
        }
        if (GeoUtils.hasCoordinate(longitude, latitude)) {
    ...
    public void refreshAveragePrices(Collection<Long> merchantIds) {
        ...
        Map<Long, OrderClient.CompletedOrderStats> completedStats = orderClient.getCompletedOrderStats(ids);
