...
public class RoleApplicationService {
    ...
    public List<RoleApplicationRecordResponse> listMineRecords(String userId) {
        List<RoleApplicationRecordResponse> records = new ArrayList<>(compatClient.listRiderApplicationRecords(userId));
        for (RoleApplication application : listMine(userId)) {
            if (RIDER.equals(application.getTargetRole()) && !records.isEmpty()) {
                continue;
            }
